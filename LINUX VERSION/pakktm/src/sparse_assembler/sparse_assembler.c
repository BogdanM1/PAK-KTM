...
void rb_insertSub(RBNode_t *x)
{
	...
	insert2Sub(x);
	x->color = red;
	while ( (x!=(rootSub)) && ( x->father->color==red) ) 
		if (x->father==x->father->father->leftson) {
			y = x->father->father->rightson;
			if (y->color==red) {
				x->father->color = black;        /*  Fall 1  */
				...
				x->father->color = black;        /*  Fall 3  */ 
				x->father->father->color = red;  /*          */
				rightrotateSub(x->father->father);           /*          */
			}
		}
		else {
			y = x->father->father->leftson;
...
/* entfernt *z im Baum */
{
	...
	if (y->color==black) rb_delete_fixupSub(x);
	free(y);
}
...
{
	RBNode_t *w;
	while ( (x!=rootSub) && (x->color==black) )
		if (x==x->father->leftson) {
			w = x->father->rightson;
			if (w->color==red) {
				w->color = black;                /*  Fall 1  */
				x->father->color = red;          /*          */
				leftrotateSub(x->father);                 /*          */
				w = x->father->rightson;                  /*          */
			}
			if (    (w->leftson->color==black) 
				&& (w->rightson->color==black) ) {
...
void AddValSub(int row, int col, Tinfo val)
{
	RBNode_t * pNode;
	Tkey index;
	if (val == 0) return;
	*(((int*)&index) + 1) = row;
	*(int*)&index = col;
	pNode = search(rootSub, index);
	if (pNode->key == index)
	{
		pNode->info += val;
		if (pNode->info == 0) rb_deleteSub(pNode);
	}
	else insertSub(index, val);
}
...
void SetValSub(int row, int col, Tinfo val)
{
	RBNode_t * pNode;
	Tkey index;
	if (val == 0) return;
...
// For GFORTRAN: void __cdecl sparseassembler_kill()
 ...
 void __cdecl SPARSEASSEMBLER_KILL(int inff)
#else
  void sparseassembler_kill_(int inff)
#endif
{
    if(inff == 1) printf("Killing Sparse Assembler...\n");
	clean(root);
}

#if defined (_WIN32) || defined (_WIN64)
 void __cdecl SPARSEASSEMBLER_KILL_SUB(int inff)
#else
  void sparseassembler_kill_sub_(int inff)
#endif
{
    if(inff == 1) printf("Killing Sparse Assembler...\n");
	clean(rootSub);
...
#if defined (_WIN32) || defined (_WIN64)
  void __cdecl SPARSEASSEMBLER_INIT(int *symetric,int inff)
#else
  void  sparseassembler_init_(int *symetric, int inff)
#endif
{
	if(inff == 1)  printf("Initializing Sparse Assembler...\n");
	RBInit();
	bSymetric = *symetric;
}

#if defined (_WIN32) || defined (_WIN64)
  void __cdecl SPARSEASSEMBLER_INIT_SUB(int *symetric,int inff)
#else
  void  sparseassembler_init_sub_(int *symetric, int inff)
#endif
{
	if(inff == 1)  printf("Initializing Sparse Assembler...\n");
	RBInitSub();
...
#if defined (_WIN32) || defined (_WIN64)
    void __cdecl SPARSEASSEMBLER_ADDELEMMATRIX_SUB(int *n, int *indices, double *vals)
#else
  	void sparseassembler_addelemmatrix_sub_(int *n, int *indices, double *vals)
#endif
{
    int i,j, nn = *n;
...
//    		        AddVal(indices[i], indices[j], vals[i*nn+j]);
//	            else
			        AddValSub(indices[j], indices[i], vals[i*nn+j]);
            }
		}
	}
}
...
#if defined (_WIN32) || defined (_WIN64)
    void __cdecl SPARSEASSEMBLER_GETSPARSE(int *nz, int *rows, int *cols, double *vals, int inff)
#else
  	void sparseassembler_getsparse_(int *nz, int *rows, int *cols, double *vals, int inff)
#endif
{
	packRow = rows;
	packCol = cols;
	...
	inorder(root);
    *nz = snimout;
    if (inff == 1) printf("Nonzero count: %d\n", *nz);
}
#if defined (_WIN32) || defined (_WIN64)
    void __cdecl SPARSEASSEMBLER_GETSPARSE_SUB(int *nz, int *rows, int *cols, double *vals, int inff)
#else
  	void sparseassembler_getsparse_sub_(int *nz, int *rows, int *cols, double *vals, int inff)
#endif
{
	packRow = rows;
	packCol = cols;
...
#else
   void sparseassembler_getnonzero_(int *nz)
#endif
{   
	*nz = RBCount(root);
}
...
#if defined (_WIN32) || defined (_WIN64)
   void __cdecl SPARSEASSEMBLER_GETNONZERO_SUB(int *nz)
#else
   void sparseassembler_getnonzero_sub_(int *nz)
#endif
{ 
	*nz = RBCount(rootSub);
}
...
#else
   void sparseassembler_setval_(int *row, int *col, double *val)
#endif
{
	SetVal(*row, *col, *val);
}
...
#if defined (_WIN32) || defined (_WIN64)
   void __cdecl SPARSEASSEMBLER_SETVAL_SUB(int *row, int *col, double *val)
#else
   void sparseassembler_setval_sub_(int *row, int *col, double *val)
#endif
{
	SetValSub(*row, *col, *val);
}

#if defined (_WIN32) || defined (_WIN64)
  void __cdecl SPARSEASSEMBLER_ZERO()
#else
  	void sparseassembler_zero_()
#endif
{
	SetAllZero();
}
...
#if defined (_WIN32) || defined (_WIN64)
  void __cdecl SPARSEASSEMBLER_ZERO_SUB()
#else
  	void sparseassembler_zero_sub_()
#endif
{
	SetAllZeroSub();
}
...
#else
  	void sparseassembler_savesparsefile_()
#endif
{
...
#if defined (_WIN32) || defined (_WIN64)
    void __cdecl SPARSEASSEMBLER_SAVESPARSEFILE_SUB()
#else
  	void sparseassembler_savesparsefile_sub_()
#endif
{
	rb_callback_func = SnimiElem_u_Fajl_callback;
    if( (SparseStream  = fopen( SparseFileName, "wb" )) == NULL )
